...
public:
    ...
    void set_title() {
        ...
        XMapWindow(dpy, win);
        XStoreName(dpy, win, "Space Shooter 7");
    }
...
void physics()
{
    ...
    if (gl.keys[XK_Left] && gl.keys[XK_Right]){
            g.ship.vel[0] = 0;
    } else if (gl.keys[XK_Left] || gl.keys[XK_Right]){
        if (gl.keys[XK_Left]) {
       ...
       shipStop.easeX(g.ship.vel);
    }
    if (gl.keys[XK_Up] && gl.keys[XK_Down]){
        g.ship.vel[1] = 0;
    } else if (gl.keys[XK_Up] || gl.keys[XK_Down]){
        if (gl.keys[XK_Up]) {
...
void render()
{
    ...
    if (menu.menu_flag) {
    //Game starts once player has pressed "s"
    //-------------------------------------------------------------------------
    //menu.btate(menu.credits_flag, menu.help_flag, menu.menu_flag);
        Rect r;
        glClear(GL_COLOR_BUFFER_BIT);
